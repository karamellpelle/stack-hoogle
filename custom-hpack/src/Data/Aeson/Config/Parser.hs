...
fromAesonPathElement e = case e of
  Aeson.Key k -> Key $ Aeson.toText k
  ...
  where
    ...
    go path value
      ...
      | otherwise = case value of
          ...
          Null -> return ()
          Object o -> do
            forM_ (Aeson.toList o) $ \ (k, v) -> do
              unless ("_" `T.isPrefixOf` (Aeson.toText k)) $ do
                go (Key (Aeson.toText k) : path) v
          Array xs -> do
...
explicitParseField p o key = case Aeson.lookup (Aeson.fromText key) o of
  Nothing -> fail $ "key " ++ show key ++ " not present"
  Just v  -> p v <?> Aeson.Key (Aeson.fromText key)

explicitParseFieldMaybe :: (Value -> Parser a) -> Object -> Text -> Parser (Maybe a)
explicitParseFieldMaybe p o key = case Aeson.lookup (Aeson.fromText key) o of
  Nothing -> pure Nothing
  Just v  -> Just <$> p v <?> Aeson.Key (Aeson.fromText key)
